// Grab the Vwidth of the containing box
var Vwidth = parseInt(d3.select("#scatter").style("Vwidth"));

// Designate the Vheight of the graph
var Vheight = Vwidth - Vwidth / 3.9;
...
function xTextRefresh() {
  xText.attr(
    "transform",
    "translate(" +
      ((Vwidth - labelArea) / 2 + labelArea) +
      ", " +
      (Vheight - margin - tPadBot) +
      ")"
  );
}
xTextRefresh();
...
var leftTextY = (Vheight + labelArea) / 2 - labelArea;
...
function visualize(theData) {
  ...
  var xScale = d3
    .scaleLinear()
    .domain([xMin, xMax])
    .range([margin + labelArea, Vwidth - margin]);
  var yScale = d3
    .scaleLinear()
    .domain([yMin, yMax])
    // Vheight is inverses due to how d3 calc's y-axis placement
    .range([Vheight - margin - labelArea, margin]);
  ...
  svg
    ...
    .call(xAxis)
    .attr("class", "xAxis")
    .attr("transform", "translate(0," + (Vheight - margin - labelArea) + ")");
  svg
    .append("g")
    .call(yAxis)
    .attr("class", "yAxis")
  ...
  theCircles
    ...
    .attr("dy", function(d) {
      ...
      // adding a third of the radius to the Vheight
  ...
  function resize() {
    // Redefine the Vwidth, Vheight and leftTextY (the three variables dependent on the Vwidth of the window).
    Vwidth = parseInt(d3.select("#scatter").style("Vwidth"));
    Vheight = Vwidth - Vwidth / 3.9;
    leftTextY = (Vheight + labelArea) / 2 - labelArea;

    // Apply the Vwidth and Vheight to the svg canvas.
    svg.attr("Vwidth", Vwidth).attr("Vheight", Vheight);

    // Change the xScale and yScale ranges
    xScale.range([margin + labelArea, Vwidth - margin]);
    yScale.range([Vheight - margin - labelArea, margin]);

    // With the scales changes, update the axes (and the Vheight of the x-axis)
    svg
      .select(".xAxis")
      .call(xAxis)
      .attr("transform", "translate(0," + (Vheight - margin - labelArea) + ")");
